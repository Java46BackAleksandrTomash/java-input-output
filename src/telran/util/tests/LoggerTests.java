...
class LoggerTests {
	...
	void testWarn() {
		runLogger(WARN);
		set = getLogsLevels();
		assertEquals(set, Set.of(ERROR, WARN));
	}
	...
	void testInfo() {
		runLogger(INFO);
		set = getLogsLevels();
		assertEquals(set, Set.of(ERROR, WARN, INFO));
	}
	...
	void testDebug() {
		runLogger(DEBUG);
		set = getLogsLevels();
		assertEquals(set, Set.of(ERROR, WARN, INFO, DEBUG));
	}
	...
	void testTrace() {
		runLogger(TRACE);
		set = getLogsLevels();
		assertEquals(set, Set.of(ERROR, WARN, INFO, DEBUG, TRACE));
	}
